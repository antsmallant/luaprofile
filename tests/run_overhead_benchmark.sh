...
REPORT_FILE="$REPORT_DIR/overhead-$PROFILE.json"
LUA_SRC_DIR="$ROOT_DIR/3rd/lua-5.4.8/src"
LUA_BIN="$LUA_SRC_DIR/lua"

if [[ "$PROFILE" != "smoke" && "$PROFILE" != "extended" ]]; then
    echo "usage: bash tests/run_overhead_benchmark.sh [smoke|extended]" >&2
    exit 2
fi
...
"$LUA_BIN" "$ROOT_DIR/tests/overhead_benchmark.lua" "$BUILD_DIR" "$ROOT_DIR" "$REPORT_FILE" "$PROFILE"
"$LUA_BIN" "$ROOT_DIR/tests/validate_overhead_report.lua" "$REPORT_FILE"

echo "overhead benchmark $PROFILE passed: $REPORT_FILE"
